...
namespace Huddled.Wpf
{
	public class MetroWindow : NativeBehavior
	{
		...
		public static readonly DependencyProperty ButtonSizeProperty = DependencyProperty.Register(
			...
			typeof(double),
			typeof(MetroWindow),
			new PropertyMetadata(SystemParameters.CaptionHeight - 4));
		...
		{
			MinimizeButton = (Button) AssociatedObject.Template.FindName("MinimizeButton", AssociatedObject);
			MinimizeButton.Click += OnMinimizeClick;
			MaximizeButton = (Button) AssociatedObject.Template.FindName("MaximizeButton", AssociatedObject);
			MaximizeButton.Click += OnMaximizeClick;
			CloseButton = (Button)AssociatedObject.Template.FindName("CloseButton", AssociatedObject);
		...
		private IntPtr _HandleNCHitTest(IntPtr wParam, IntPtr lParam, ref bool handled)
		{
			...
			if (IntPtr.Zero == lRet)
			{
				...
				if (ht == NativeMethods.HT.CAPTION)
				{
					...
					ht = _HitTestUIElements(mousePosWindow);
				}
				handled = NativeMethods.HT.NOWHERE != ht;
				lRet = new IntPtr((int)ht);
				Debug.WriteLine("NC HitTest: " + ht);
			}
			return lRet;
		}
		...
		private NativeMethods.HT _HitTestUIElements(Point mousePosWindow)
		{
			var ret = NativeMethods.HT.CAPTION;
			VisualTreeHelper.HitTest(
				AssociatedObject,
				target =>
				{
					UIElement uie = target as UIElement;
					//if(uie == MinimizeButton)
					//{
					//    ret = NativeMethods.HT.MINBUTTON;
					//    return HitTestFilterBehavior.Stop;
					//}
					//if (uie == MaximizeButton)
					//{
					//    ret = NativeMethods.HT.MAXBUTTON;
					//    return HitTestFilterBehavior.Stop;
					//}
					//if (uie == CloseButton)
					//{
					//    ret = NativeMethods.HT.CLOSE;
					//    return HitTestFilterBehavior.Stop;
					//}

					if (null != uie && MetroWindow.GetHitTestable(uie))
					{
						ret = NativeMethods.HT.NOWHERE;
						return HitTestFilterBehavior.Stop;
					}
		...
		private void InitializeDockedViewStates()
		{
			...
			((ThicknessAnimation)AssociatedObject.Template.FindName("DockedFullHeightThickness", AssociatedObject)).To = originalMargin.Clone(top: 0, bottom: 0);
			((ThicknessAnimation)AssociatedObject.Template.FindName("DockedFullHeightBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(top: 0, bottom: 0);

			((ThicknessAnimation)AssociatedObject.Template.FindName("DockedFullLeftThickness", AssociatedObject)).To = originalMargin.Clone(left: 0, top: 0, bottom: 0);
			((ThicknessAnimation)AssociatedObject.Template.FindName("DockedFullLeftBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(left: 0, top: 0, bottom: 0);
			...
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedRightThickness", AssociatedObject)).To = originalMargin.Clone(right: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedRightBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(right: 0);

			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedTopThickness", AssociatedObject)).To = originalMargin.Clone(top: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedTopBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(top: 0);

			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedBottomThickness", AssociatedObject)).To = originalMargin.Clone(bottom: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedBottomBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(bottom: 0);

			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedTopLeftThickness", AssociatedObject)).To = originalMargin.Clone(left: 0, top: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedTopLeftBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(left: 0, top: 0);

			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedTopRightThickness", AssociatedObject)).To = originalMargin.Clone(top: 0, right: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedTopRightBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(top: 0, right: 0);

			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedBottomLeftThickness", AssociatedObject)).To = originalMargin.Clone(left: 0, bottom: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedBottomLeftBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(left: 0, bottom: 0);

			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedBottomRightThickness", AssociatedObject)).To = originalMargin.Clone(bottom: 0, right: 0);
			//((ThicknessAnimation)AssociatedObject.Template.FindName("DockedBottomRightBorder", AssociatedObject)).To = AssociatedObject.Margin.Clone(bottom: 0, right: 0);
		}
