...
function Global:Write-BootsOutput($inputObject) {
   ...
   PROCESS {
      if($_) {
        ...
        {
            List<ScriptBlock> scriptContent = new List<ScriptBlock>(Content);
            ...
            _dispatcher.Invoke((Action)(() =>
            {
                try
                {
                    if (scriptContent.Count > 0)
                    {
                        ...
                        if (_window != null)
                        {
                           SetScriptVariableValue("this", _window);
                           SetScriptVariableValue("window", _window);
                        }
                        foreach (var pso in scriptContent)
                        {
                            scriptResults.AddRange(Invoke(pso, Parameters));
                        }
